			...
			if ( $trip['@attributes']['id'] === $tripId ) {
				...
				if ( is_array($departures) ) {
					foreach ( $departures as $departure ) {
						if ( isset($departure['Prices']) ) {
							$prices = $departure['Prices'];
						}
						// $departureList = array(
						// 	// 'startDate' 	=> date_format(date_create($departure['@attributes']['startDate']), "j M Y"),
						// 	// 'endDate' 		=> date_format(date_create($departure['@attributes']['endDate']), "j M Y"),
						// 	// 'Availability' 	=> $departure['@attributes']['availableSpaces'],
						// 	// 'departId' 		=> $departure['@attributes']['id'],
						// 	'Departures'	=> $departure
						// 	// 'prices'		=> $prices['Price']
						// );
						
						// $data[]['Departure'] = $departureList;
						$data['Departures'][] = $departure;
					}
					$data['PillonFriendly'] = $trip['Templates']['Template']['@attributes']['isPillionFriendly'];
				}
			}
		}
